import type { Metadata } from 'next';
import { Building2, Smartphone, Briefcase, ChevronRight } from 'lucide-react';
import type { LucideIcon } from 'lucide-react';
import Link from 'next/link';
import { getTranslations } from 'next-intl/server';

const ICONS: Record<string, LucideIcon> = {
  fintech: Building2,
  telco: Smartphone,
  hr: Briefcase,
};

type Industry = {
  id: string;
  name: string;
  description: string;
  benefits: string[];
};

export async function generateMetadata(): Promise<Metadata> {
  const t = await getTranslations('casosPage');
  return {
    title: t('metaTitle'),
    description: t('metaDescription'),
  };
}

export default async function CasosPage() {
  const t = await getTranslations('casosPage');
  const industries = t.raw('items') as Industry[];

  return (
    <div className="bg-white py-24 sm:py-32">
      <div className="mx-auto max-w-7xl px-6 lg:px-8">
        <div className="mx-auto max-w-2xl lg:text-center">
          <h2 className="text-base font-semibold leading-7 text-blue-600">{t('kicker')}</h2>
          <p className="mt-2 text-3xl font-bold tracking-tight text-primary sm:text-4xl">{t('title')}</p>
          <p className="mt-6 text-lg leading-8 text-gray-600">{t('sub')}</p>
        </div>

        <div className="mx-auto mt-16 max-w-2xl sm:mt-20 lg:mt-24 lg:max-w-none">
          <div className="grid grid-cols-1 gap-x-8 gap-y-16 lg:grid-cols-3">
            {industries.map((industry) => {
              const Icon = ICONS[industry.id] ?? Building2;
              return (
                <div key={industry.id} className="flex flex-col bg-gray-50 rounded-2xl p-8 border border-gray-100">
                  <div className="flex items-center gap-x-3 text-xl font-bold leading-7 text-gray-900 mb-4">
                    <Icon className="h-8 w-8 text-blue-600" aria-hidden="true" />
                    {industry.name}
                  </div>
                  <p className="text-base leading-7 text-gray-600 mb-6 flex-auto">{industry.description}</p>
                  <div className="mb-8">
                    <h4 className="font-semibold text-sm text-gray-900 mb-3">{t('benefits')}</h4>
                    <ul className="space-y-2">
                      {industry.benefits.map((benefit) => (
                        <li key={benefit} className="text-sm text-gray-600 flex items-center">
                          <ChevronRight className="h-4 w-4 text-blue-500 mr-2" />
                          {benefit}
                        </li>
                      ))}
                    </ul>
                  </div>
                  <Link href="/contacto" className="mt-auto text-sm font-semibold text-primary hover:text-blue-600">
                    {t('demo', { name: industry.name })} <span aria-hidden="true">&rarr;</span>
                  </Link>
                </div>
              );
            })}
          </div>
        </div>
      </div>
    </div>
  );
}
